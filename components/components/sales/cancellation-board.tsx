                    ...
                    */}
                    {preview.serviceFinding.blocking ? (
                      ...
                      <div
                        ...
                        data-word={preview.serviceFinding.word}
                      >
                        <p>{preview.serviceFinding.headline}</p>
                        {/*
                          ⭐⭐ A PASSING FILE IS NOT ALWAYS A QUIET ONE — 0111.

                          🔴 THIS BRANCH USED TO PRINT THE HEADLINE ALONE,
                          and the headline of a passing finding is one
                          reassuring sentence. Two things now travel in
                          `detail` that a passing file can carry and that
                          the person about to forfeit somebody's deposit
                          has to read anyway:

                            • rungs served by DEEMING rather than by proof
                              — served in law, no delivery anybody watched,
                              and the first thing the allottee's advocate
                              attacks;
                            • rungs raised before this system recorded
                              which right they were raised under.

                          Neither blocks. Both change what the file is
                          worth in a hearing, so showing only the green
                          sentence would be the same silence 0098 removed
                          from `sent_at` — one screen further along.
                        */}
                        {preview.serviceFinding.deemedStages.length > 0 ||
                        preview.serviceFinding.unrecordedAuthorityStages.length > 0 ? (
                          <p className="mt-1">{preview.serviceFinding.detail}</p>
                        ) : null}
                      </div>
                    )}
