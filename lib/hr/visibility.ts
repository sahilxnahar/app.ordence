...
export function canReadReview(
  ...
  rel: ViewerRelation,
  args: { released: boolean; submitted: boolean },
): boolean {
  if (rel.isHr) return true;

  // ⚠️ DRAFTS ARE PRIVATE. If it's not submitted yet, only the person
  // who is allowed to write it can read it.
  if (!args.submitted) {
    return canWriteReview(kind, rel);
  }
