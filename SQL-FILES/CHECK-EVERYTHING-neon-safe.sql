-- =====================================================================
--  ORDENCE , ONE FILE THAT CHECKS EVERYTHING
--  Read only. SAFE AGAINST NEON. Safe on production. Nothing is written.
--  Covers migrations 0001 to 0090, tenant isolation, and the connection.
-- =====================================================================
--
--  Paste the whole file into the Neon SQL editor and press Run. It
--  returns five result tabs. Read them in order.
--
--    1. MIGRATIONS , how many applied, how many left, the next to run
--    2. WHAT TO RUN , the exact files, in the exact order. Empty = done.
--    3. GAPS , a file skipped while a later one was applied. Worse than
--       a backlog, because something ran out of sequence.
--    4. TENANT ISOLATION , every tenant table has RLS enabled, forced,
--       and a policy. This is the check that catches `drizzle-kit push`.
--    5. THE CONNECTION , rolsuper and rolbypassrls for this role.
--
--  HOW THE MIGRATION CHECK DECIDES. There is no migrations ledger in this
--  product; the files are run by a human. So for each migration this
--  asks whether the object that migration CREATES exists right now. Each
--  is matched to an object only that migration creates, so nothing can be
--  reported applied because a different file made the same thing.
--
--  ⚠️ IT CANNOT PROVE A MIGRATION RAN COMPLETELY. A file that created its
--  first table then failed on statement forty reads as APPLIED. That is
--  what each VERIFY-00NN-neon-safe.sql is for.
--
--  🔴 NEVER RUN A FILE NAMED DRILL-DO-NOT-RUN-IN-NEON-*.sql HERE.
-- =====================================================================


-- ---------------------------------------------------------------------
--  1. MIGRATIONS , the headline.
-- ---------------------------------------------------------------------
WITH expected (num, file_name, signature, present) AS (
  VALUES
  ('0001', '0001_rls_and_audit_guard.sql', 'function app_current_tenant_id()', (EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='app_current_tenant_id'))),
  ...
  ('0033', '0033_engine1_scheduling.sql', 'column schedule_bookings.capacity_hint', (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='schedule_bookings' AND column_name='capacity_hint'))),
  ...
  ('0086', '0086_policy_platform_platform_tables.sql', 'policy tenant_health_events_platform_only on tenant_health_events', (EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='tenant_health_events' AND policyname='tenant_health_events_platform_only'))),
  ('0087', '0087_hardening_narrow_grants.sql', 'EXECUTE on indian_financial_year() for ordence_app', (EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='indian_financial_year' AND has_function_privilege('ordence_app', p.oid, 'EXECUTE')))),
  ('0088', '0088_hardening_auth_events.sql', 'enum value security_event_type.auth.account_locked', (EXISTS (SELECT 1 FROM pg_type t JOIN pg_enum e ON e.enumtypid=t.oid WHERE t.typname='security_event_type' AND e.enumlabel='auth.account_locked'))),
  ('0089', '0089_hardening_login_lockouts.sql', 'table login_lockouts', (to_regclass('public.login_lockouts') IS NOT NULL)),
  ('0090', '0090_period_close_message_normalization.sql', 'function enforce_period_close()', (EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='enforce_period_close')))
)
SELECT
  '1. MIGRATIONS' AS section,
  count(*)                                              AS in_this_release,
  count(*) FILTER (WHERE present)                       AS applied,
  count(*) FILTER (WHERE NOT present)                   AS still_to_run,
  coalesce(max(num) FILTER (WHERE present), 'none')     AS highest_applied,
  coalesce(min(num) FILTER (WHERE NOT present), 'none') AS first_to_run
FROM expected;


-- ---------------------------------------------------------------------
--  2. EXACTLY WHAT TO RUN, IN ORDER. Empty result = fully up to date.
-- ---------------------------------------------------------------------
WITH expected (num, file_name, signature, present) AS (
  VALUES
  ('0001', '0001_rls_and_audit_guard.sql', 'function app_current_tenant_id()', (EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='app_current_tenant_id'))),
...
FROM expected WHERE NOT present ORDER BY num;


-- ---------------------------------------------------------------------
--  3. GAPS. A missing file with a LATER one applied. Read before running.
-- ---------------------------------------------------------------------
WITH expected (num, file_name, signature, present) AS (
  VALUES
  ('0001', '0001_rls_and_audit_guard.sql', 'function app_current_tenant_id()', (EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='app_current_tenant_id'))),
...
SELECT
  CASE WHEN count(*) = 0
    THEN 'No gaps. Anything missing is the ordinary backlog at the end.'
    ELSE '🔴 GAP , ' || count(*)::text || ' file(s) were skipped. They are listed below.'
  END AS gap_check
FROM expected e
WHERE NOT e.present AND EXISTS (SELECT 1 FROM expected l WHERE l.present AND l.num > e.num);


-- ---------------------------------------------------------------------
--  4. TENANT ISOLATION. The check that catches a stripped database.
--     Eight platform-evidence tables are exempt by name; that list is
--     `OPT_IN_PLATFORM_WRITE` in scripts/check-rls-coverage.mjs, copied
--     verbatim. They carry a tenant_id but are scoped by
--     app_platform_scope() on purpose. A NEW name appearing here is real.
-- ---------------------------------------------------------------------
WITH scoped AS (
  ...
  WHERE n.nspname='public' AND c.relkind='r'
    ...
    AND c.relname NOT IN ('login_lockouts','error_events','platform_entitlement_history',
...
SELECT
  ...
  count(*) FILTER (WHERE rls_enabled AND NOT rls_forced) AS not_forced,
  count(*) FILTER (WHERE rls_enabled AND NOT has_tenant_policy) AS no_policy,
  CASE WHEN count(*) FILTER (WHERE NOT (rls_enabled AND rls_forced AND has_tenant_policy)) = 0
    THEN 'ALL CLEAR , tenant isolation intact on every tenant table.'
    ELSE '🔴 BREACH , ' || count(*) FILTER (WHERE NOT (rls_enabled AND rls_forced AND has_tenant_policy))::text
         || ' table(s) unprotected. Treat as a live incident and tell Claude.'
  END AS verdict
FROM judged;


-- ---------------------------------------------------------------------
--  5. THE CONNECTION. True on either flag and section 4 means nothing:
--     this role ignores every policy in the product.
-- ---------------------------------------------------------------------
